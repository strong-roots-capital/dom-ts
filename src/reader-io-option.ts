...
export const fromIOOption: <R, A>(ma: IOO.IOOption<A>) => ReaderIOOption<R, A> = M.fromM
export const fromReader: <R, A>(r: R.Reader<R, A>) => ReaderIOOption<R, A> = M.fromReader
...
export const map = <A, B>(f: (a: A) => B) => <R>(fa: ReaderIOOption<R, A>): ReaderIOOption<R, B> =>
  M.map(fa, f)
